...
{ 
   ...
   // pthread_mutex_lock(&gm);
    for (int i = 0; i < 4; i++)
    {
        key = rand()%(VAL_MAX); //0-9 if max = 10
        v = rand()%(26) + 65;  //A-Z        
        ht_add(key, v);
    }  
...
int main()
{
    ...
    else
    {
       ...
       //init hash table
        cout<<"initial linear probed hashtab:"<<endl;
        ht_init();
        ...
        pthread_create(&tid1, NULL, tajh, NULL);
        ...
        for( i=0; i < number_of_threads; i++ )
        {
            ...
            {
                td[i].thread_op = 0;
                td[i].thread_key = rand()%(VAL_MAX);  //0 to 9
                td[i].thread_val = rand()%26 + 65;//value to be inserted: A-Z
                val = td[i].thread_val;
            }
