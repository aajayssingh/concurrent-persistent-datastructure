...
public:
    ...
    static inline unsigned long my_custom_nvrecover(void *dest, unsigned long pageNo, unsigned long pageOffset, size_t size)
    {
        unsigned long addr;
        addr = xmemory::_localNvRecovery->my_custom_nvrecover(dest, pageNo,pageOffset, size);
        return addr;
    }
