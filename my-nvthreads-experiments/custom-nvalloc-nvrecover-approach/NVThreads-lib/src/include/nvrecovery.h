...
/*
* writing my own customised nvrecover to recover the nodes of a linked list.
*/
unsigned long my_custom_nvrecover(void *dest, unsigned long pageNo, unsigned long pageOffset, size_t size) {
        ...
        //allocate varmap and populate partially to enable maximum code resuse
        v = (struct varmap_entry *)malloc(sizeof(struct varmap_entry));
        if(v != NULL){
        ...
        {
            printf("Oh mon Dieu, cannot allocate custom varmap entry\n");
        }
