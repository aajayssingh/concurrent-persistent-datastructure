...
void show_arr()
{
  for(int i = 0; i < VAL_MAX; i++)
  {
    printf("arr[%d]-%d : ",i, pers_ptr_arr[i]);
  }
  printf("\n");
}
...
list_t *init_list(void)
{
  ...
  // pers_ptr_arr = (int *)nvmalloc(VAL_MAX*sizeof(int), (char *)"arr");
  // init_arr();
...
strcpy(list->tailNodeName, list->tail->nvnext.selfNodeName);
  ...
  // pers_ptr_arr[VAL_MIN] = VAL_MIN;
  // pers_ptr_arr[VAL_MAX] = VAL_MAX;
...
list_t * recover_init_list (void)
{
  ...
  // pers_ptr_arr = (int *)malloc(VAL_MAX*sizeof(int));
  // nvrecover(pers_ptr_arr, VAL_MAX*sizeof(int), (char*)"arr");
  //show_arr();
