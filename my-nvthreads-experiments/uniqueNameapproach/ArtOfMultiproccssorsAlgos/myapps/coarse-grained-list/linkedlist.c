...
static node_t *create_node(const key_t key, const val_t val, list_t *list)
{
  ...
  /* Give unique names to the nodes, these names are handles using which recover is done using nvrecover()*/
...
bool add(list_t * list, const key_t key, const val_t val)
{
  ...
  //pthread_mutex_lock(&(list->gmtx));
    ...
    {
      if ((newNode = create_node(key, val, list)) == NULL)
      {
 ...
 // pthread_mutex_lock(&(list->gmtx));
  ...
  if (key == curr->key)
  {
    *val = curr->val;      
    ...
    list->real_node_count = list->real_node_count - 1;//keeps track of actual nodes in the list
...
list_t *init_list(void)
{
  ...
  /*initial head point to tail*/
  updateLinks(list->head, list->tail);

  //init global list mutex lock used for coarse grained list impl
  pthread_mutex_init(&(list->gmtx), NULL);
  printf("inited list->gmtx: %p\n", &(list->gmtx));
