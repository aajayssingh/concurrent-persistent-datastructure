...
static node_t *create_node(const key_t key, const val_t val, list_t *list)
{
 ...
 // printf("created node\n");  
  ...
  {
    node->selfNodeName[i] = name[i];
    node->nextNodeName[i] = 'A';  
  }
  node->next = NULL;
...
void updateLinks(node_t* first, node_t* second)
{
  for(int i = 0; i < NODE_INFO_ARR_SIZE; i++)
  {
      first->nextNodeName[i] = second->selfNodeName[i];
  }
  first->next = second;
}
...
bool add(list_t * list, const key_t key, const val_t val)
{
 ...
 /* if ((newNode = create_node(key, val)) == NULL)
  ...
  nvcheckpoint();
  if(ret)
    printf("add [%d, %d], %s, %s, %s, %s, %s\n", key, val, pred->selfNodeName, pred->nextNodeName, newNode->selfNodeName, newNode->nextNodeName, curr->selfNodeName);
...
list_t *init_list(void)
{
  ...
  {
    list->headNodeName[i] = list->head->selfNodeName[i];
  }  
  ...
  for(int i = 0; i < NODE_INFO_ARR_SIZE; i++)
  {
    list->tailNodeName[i] = list->tail->selfNodeName[i];
  }
...
void printNodeInfo(node_t * node)
{
  ...
  printf("node->selfNodeName: %s\n", node->selfNodeName);
  printf("node->nextNodeName: %s\n\n", node->nextNodeName);
  #endif
}
...
list_t * recover_init_list (void)
{
  ...
  {    
    ...
    nvrecover(curr, sizeof(node_t), pred->nextNodeName);
...
void show_list_with_offsets(const list_t * l)
{
    ...
    printf ("list:\n\n");
    printf(" -->[%ld:%ld]: %s %s\n", (long int) pred->key, (long int)pred->val, pred->selfNodeName, pred->nextNodeName);
    int node_index = 0;

    while ((curr != l->tail)&&(++node_index < l->real_node_count - 1)) {
      printf(" -->curr[%ld:%ld]: pred:%s %s :: curr:%s %s      ->%d\n", (long int) curr->key, (long int)curr->val, pred->selfNodeName, pred->nextNodeName, curr->selfNodeName, curr->nextNodeName, node_index);

    if(curr->key == 0 && curr->val == 0)
      printf("----->");
    if(strcmp(pred->nextNodeName, curr->selfNodeName) != 0)
      printf("***blunder next pointer and offsets not synchronized***");

      pred = curr;
      curr = curr->next;
    }
    printf(" -->[%ld:%ld]: %s %s\n", (long int) curr->key, (long int)curr->val, curr->selfNodeName, curr->nextNodeName);
    printf("********************************************************************************\n");
}
